...
def searchbywhoosh(keyword,searchdir):
    ...
    schema = Schema(title=TEXT(stored=True, analyzer=analyzer), path=ID(stored=True), html=ID(stored=True),
        ...
        for name in files:
            ...
            if filename[-5:] == '.docx':
                ...
                if os.name == 'nt':
                    title = title.decode('gb2312').encode('utf-8').decode('utf-8')
                    filename = filename.decode('gb2312').encode('utf-8').decode('utf-8')
                    html = newfilename.replace('.txt', '.htm').decode('gb2312').encode('utf-8').decode('utf-8')
                    txt = txt.decode('utf-8')
                else:
                    title = title.decode('utf-8')
                    filename = filename.decode('utf-8')
                    html = newfilename.replace('.txt', '.htm').decode('utf-8')
                    txt = txt.decode('utf-8')
                writer.add_document(title=title, path=filename, html=html, content=txt)

    # print txt,txt2html(txt)

    writer.add_document(title=u"第一篇文档", path=u"/a", html=u'/a',
                        content=u"这是我们增加的第一篇文档")
    writer.add_document(title=u"第二篇文档", path=u"/b", html=u'/b',
    ...
    with ix.searcher() as searcher:
        ...
        for i, result in enumerate(results):
            lines[i] = u'<div><p><a href="' + result['html'] + u'">' + result['title'] + u'</a>    <a href="' + result[
                'path'] + u'">源文件链接<a></p>' + u'<p>' + result.highlights('content') + u'</p></div>'
        ...
        htmlresultfile = os.path.join(docdir, keyword + '.html')
        output_file = open(htmlresultfile, 'w')
        output_file.write(html)
        output_file.close()
    print html
    print htmlresultfile
    return  htmlresultfile

if __name__ == '__main__':
    htmlresultfile = searchbywhoosh(u'设备线夹',sys.path[0])
    import webbrowser
