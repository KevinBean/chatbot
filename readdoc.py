...
def docx2html_utf8(filename):
    ...
    if os.name == 'nt':
        ...
        input_file = open(newfilename, 'r')
        gbktxt = input_file.read()
        utftxt = gbktxt.encode('utf-8') #读取txt文件，将gbk转换成utf-8
        input_file.close()
        output_file = open(newfilename, 'w')
        output_file.write(utftxt) #保存utf-8文本
        output_file.close()
