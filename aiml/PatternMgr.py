...
class PatternMgr:
	...
	def match(self, pattern, that, topic):
		...
		if topic.strip() == u"": topic = u"ULTRABOGUSDUMMYTOPIC" # 'topic' must never be empty 处理topic
	...
	def _match(self, words, thatWords, topicWords, root):
		"""
		输入words,作为that、topic的输入thatwords和topicwords
		...
		matched template.
		返回一个元组(pat, tem),其中pat是一串nodes,starting
		at the root and leading to the matching pattern。而tem是匹配的模板
		...
		if len(words) == 0:
			...
			template = None
			if len(thatWords) > 0:
				# If thatWords isn't empty, recursively 递归处理
