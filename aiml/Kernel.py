...
class Kernel:
    ...
    def respond(self, input, sessionID = _globalSessionID):
        ...
        # release the lock and return 解除锁定,返回数据
    ...
    def _respond(self, input, sessionID):
        ...
        # guard against infinite recursion 避免无限循环
        ...
        # push the input onto the input stack 处理input堆栈
        inputStack = self.getPredicate(self._inputStack, sessionID)
        inputStack.append(input)
        self.setPredicate(self._inputStack, inputStack, sessionID)

        # run the input through the 'normal' subber 使用normal替换模式替换输入文本
        subbedInput = self._subbers['normal'].sub(input)

        # fetch the bot's previous response, to pass to the match()
        # function as 'that'. 处理that标签,将之前机器人的回复传递给match()
        outputHistory = self.getPredicate(self._outputHistory, sessionID)
        try: that = outputHistory[-1]
        except IndexError: that = ""
        subbedThat = self._subbers['normal'].sub(that)

        # fetch the current topic 从session中读取现在的topic
        topic = self.getPredicate("topic", sessionID)
        subbedTopic = self._subbers['normal'].sub(topic)

        # Determine the final response. 最终的输出
        response = ""
        # self._brain = PatternMgr() match的输入为input,that和topic
        elem = self._brain.match(subbedInput, subbedThat, subbedTopic) # match最终返回的是template模版信息
        print elem
        if elem is None:
            if self._verboseMode:
            ...
            # Process the element into a response string. 将match()回复结果处理成字符串
            response += self._processElement(elem, sessionID).strip()
            response += " "
        response = response.strip()

        # pop the top entry off the input stack. input堆栈处理
        inputStack = self.getPredicate(self._inputStack, sessionID)
        inputStack.pop()
        self.setPredicate(self._inputStack, inputStack, sessionID)
        # 返回的是'bot'、'template'、'li'、'that、等的类型字符串
        return response
